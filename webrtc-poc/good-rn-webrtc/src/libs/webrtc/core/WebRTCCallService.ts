...
import { getGoogleICEServers, getOpenRelayICEServers } from '../utils/turnServerList'
...
class WebRTCCallService {
    ...
    public async handleOffer(offer: SignalingMessage) {
        ...
        try {
            ...
            await this.pc!.setRemoteDescription(new RTCSessionDescription(offer.data)).then(async () => {
                ...
                const answer = await this.pc!.createAnswer().then(async (answer) => {
            ...
            //console.log(offer.dataChannelLabel?.toChannel?.name, ' WARN --> WebRTCCallService: Error in handleOffer', e);
        }
    }
    ...
    public async handleAnswer(answer: SignalingMessage) {
        ...
        try {
            ...
            //console.log(answer.dataChannelLabel?.toChannel?.name, ' WARN --> WebRTCCallService: Error in handleAnswer', e);
        }
        //}
    }
        ...
        try {
            ...
            if (this.pc!.remoteDescription) {
                await this.pc!.addIceCandidate(new RTCIceCandidate(candidate.data)).then(() => {
                    ...
                    const msg: SignalingMessage = { type: 'ready', dataChannelLabel: this.dataChannelLabel!, data: { type: 'ready', sdp: '' } };
            ...
            //console.log(candidate.dataChannelLabel?.toChannel?.name, ' WARN --> WebRTCCallService: Error in handleCandidate', e);
        }
    }
    ...
    public async StartCall() {
        try {
            ...
            await this.pc!.createOffer(sessionConstraints).then(async (offer: RTCSessionDescription) => {
                ...
                await this.pc!.setLocalDescription(offer).then(() => {
                    ...
                    //console.log('WARN: WebRTCCallService: Error setting local description:', error);
                    return false;
                });
            });
            //}
    ...
    private async createPeerConnection() {
        try {
            ...
            var peerConfiguration: RTCConfiguration = getOpenRelayICEServers();
    ...
    private async setupDataChannel(channel: any) {
        try {
    ...
    private handleIceCandidate(e: any) {
        try {
            ...
            console.log('WARN: WebRTCCallService: Error in handleIceCandidate', error);
        }
    }

    private async receiveChannelCallback(event: any) {
        try {
    ...
    public async StartConnection() {
        // start the peer connection set0up
        await this.StartCall().then((isSuccess: boolean) => {
            if (isSuccess === false) {
                console.error('WebRTCCallService: Error in starting call for data connection');
                return false;
            }
            ...
            colabLog(this.dataChannelLabel, 'WebRTCCallService: StartConnection returning true');
            return true;
        }).catch(error => {
            console.error('WebRTCCallService: Error StartConnection:', error);
            return false;
        });
    ...
    public async StartScreenSharing() {
        ...
        await this.StartCall().then((isSuccess: boolean) => {
            if (isSuccess === false) {
                console.error('WebRTCCallService: Error in starting call for screen sharing');
                return false;
            }
            ...
            colabLog(this.dataChannelLabel, 'WebRTCCallService: screen sharing: returning true');
            return true;
        }).catch(error => {
            console.error('WebRTCCallService: Error accessing video:', error);
            return false;
        });
    }
    ...
    public StopScreenSharing() {
        if (this.localStream) {
            colabLog(this.dataChannelLabel, 'WebRTCCallService: Stopping screen sharing');

            this.isScreenSharing = false;
        }

        colabLog(this.dataChannelLabel, 'WebRTCCallService: Stopped screen sharing');
    ...
    public async StartAudioStreaming() {
        ...
        await this.StartCall().then((isSuccess: boolean) => {
            ...
            colabLog(this.dataChannelLabel, 'WebRTCCallService: audio streaming: returning true');
            return true;
        }).catch(error => {
            console.error('WebRTCCallService: Error accessing mic:', error);
            return false;
        });
    }

...
class WebRTCHandler {
	...
	signalingServerUrl: string = 'http://10.0.1.151:3030/';
	//signalingServerUrl: string = 'http://13.232.40.67:3030/';

	constructor(private cdr: ChangeDetectorRef, currentUserId: string) {
		this.currentUserId = currentUserId;
		//this.socketConnection();
	}

	public static getInstance(currentUserId: string, callback: (channelmessage: ChannelMessage) => void, commandcallback: (channelmessage: ChannelMessage) => void, cdr: ChangeDetectorRef) {
		if (WebRTCHandler.webRTCHandler === null || WebRTCHandler.webRTCHandler === undefined) {
			WebRTCHandler.webRTCHandler = new WebRTCHandler(cdr, currentUserId);
	...
	async ConnectDataChannel(dataChannelType: WebRTCDataChannelType, tenantId: number, fromChannel: Channel, toChannel: Channel, streamType: WebRTCDataChannelStreamType = WebRTCDataChannelStreamType.NONE) {
		...
		if (callServiceFacade !== null && callServiceFacade !== undefined) {
			...
			if (streamType === WebRTCDataChannelStreamType.AUDIO || streamType === WebRTCDataChannelStreamType.VIDEO) {
				console.log('createFacade: Removing existing facade for channel:', dataChannel.dataChannelName);
		...
		if (callServiceFacade === null || callServiceFacade === undefined) {
			...
			// !!!! IMP: Crete the RTCPeerConnection

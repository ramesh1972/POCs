...
function create_tab_bar(tab_container, for_section, pos)
{
	...
	var tab_bar = create_section(tab_container, "tab_bar_for_" + tab_container.id, 0, 0, w, h, "", "dock");
...
function create_tab(tab_bar, for_section)
{
	...
	set_section_dimension(for_section, 0, 0, tab_bar.tab_container.width, tab_bar.tab_container.height);
	...
	var text = create_text(tab, "tab_text_" + tab.id, tab.tab_section.id.substring(tab.tab_section.id.length-10));
...
function layout_tab_bar(tab_bar)
{
	...
	{	
		tab_bar.width = tab_bar.tab_container.width;
		...
		create_text(tab_bar.bar_left, "tit_" + tab_bar.bar_left.id, "");								
	}
	...
	{	
		tab_bar.height = tab_bar.tab_container.height;
		...
		create_text(tab_bar.bar_top, "tit_" + tab_bar.bar_top.id, "");										
	}
...
function layout_tab_bar_tabs(tab_bar)
{
	...
	else
	{
		if (tab_bar.dock_position == "top" || tab_bar.dock_position == "bottom")
			tlength = tab_bar.width/num_tabs;
		else
			tlength = tab_bar.height/num_tabs;
		...
		((tab_bar.dock_position == "left" || tab_bar.dock_position == "right") && tab_bar.height > tlength))
	{
		for (var idx=0; idx < tab_bar.child_sections.length; idx++)
		{
			...
			if (idx > 0)
				pt = tab_bar.child_sections[idx-1];
...
function set_tab_active_style(tab)
{
	...
	set_text_direction(tab, "tab_text_" + tab.id, tab.tab_bar.tab_container.tab_position);
	set_text_style(tab, "tab_text_" + tab.id, "arial", "white", 12, "bold", "yellow");	
}
...
function add_tab(for_section, tab_container)
{
	...
	set_section_parent(for_section, tab_container);
...
function add_tab_for_root_container(for_section, pos)
{
	...
	set_section_parent(for_section, tabc);
...
function remove_tab(section)
{
	...
	for (idx=0; idx < section.tab_container.child_sections.length;idx++)
	{
		var child = section.tab_container.child_sections[idx];
	...
	if (num_tabbed_sections == 1)
	{
		...
		{
			// set tab references
			section_to_adjust.tab_container = tabc_psection;
			section_to_adjust.tab = tabc.tab;
			section_to_adjust.tab.tab_section = section_to_adjust;
			section_to_adjust.is_sizable = false;
			section_to_adjust.is_visible = true;
			
			set_section_dimension(section_to_adjust, 0, 0, section_to_adjust.tab_container.width, section_to_adjust.tab_container.height);
...
function get_tab_container(section)
{
	...
	while (s != null)
	{
		if (get_parent(s) != null)
		{
			if (s.tab_container != null && 	s.tab_container.id == get_parent(s).id)
			{

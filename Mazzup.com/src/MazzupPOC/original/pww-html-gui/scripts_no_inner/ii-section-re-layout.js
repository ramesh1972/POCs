...
function border_dragend()
{
	...
	if (sectionp.position_type == "dock" || sectionp.position_type == "stack")
	{
		for (var idx=0;idx<sectionp.child_sections.length;idx++)
		{
			...
			if (top_section_exists || left_section_exists || bottom_section_exists || right_section_exists)
			{
				...
				set_section_original_dimension(section, section.width, section.height);				
				add_property(section, "is_sized", true);
				add_property(get_parent(section), "is_sized", true);
	...
	if (!found)
	{
		...
		set_section_original_dimension(sectiono, sectiono.width, sectiono.height);
...
function bar_mouseclick()
{
	...
	set_section_max_zindex(get_parent(bar));
}
...
function clone_dragdock_section(src_section)
{
	...
	gdragdock_org_section = src_section; 
	if (gdragdock_org_section.frame_handle == null)
...
function body_mousemove()
{
    ...
    if (gdragdocking)
	{
		...
		// get the deepest child section at the mouse pointer. 
		...
		if (get_parent(section) != null && 
			is_tab_bar(get_parent(section)))
			gdrop_in_section = get_parent(section);
...
function do_section_dragdrop_positioning()
{
	...
	if (gdragdock_org_section.id == gdrop_in_section.id)
		return false;
...
function dock_to_section(to_section, section, side)
{
	...
	set_section_parent(section, container);
	...
	if (side == "left" || side == "right")
	{
		...
		set_section_original_dimension(section, -1, section.height)
	}
	else if (side == "top" || side == "bottom")
	{
		section.width = to_section.width;
		set_section_original_dimension(section, section.width, -1)
	}
...
function dock_to_root_container(section, side)
{
	...
	set_section_parent(section, groot_container);
	move_last_section_to_position(section, 0);
...
function float_section(section, float_in_section, rx, ry)
{
	...
	if (get_parent(section).id == float_in_section.id)
	{
		destroy_section_borders(section);
		
		var px = gsl_s(get_parent(section));
		var py = gst_s(get_parent(section));
			
		var x = rx - px - (section.width/2);
		var y = ry - py;
			
		if (x > 0 && y  > 0)
			set_section_dimension(section, x, y, section.width, section.height);
	...
	else // if floating in a different section
	{
		if (float_in_section != null)
		{
			...
			section.position_type = "float";
			set_section_parent(section, float_in_section);

			float_in_section.is_visible = true;
...
function stick_to_section(to_section, section, side)
{
	...
	destroy_section_borders(section);
	set_section_parent(section, to_section);
...
function stack_section(section)
{
    ...
    add_property(section, "is_visible", true);

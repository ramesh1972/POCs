...
function set_section_oi_dimension(section, x, y, w, h)
{
	if (section == null)
		return;
	...
	set_section_dimension(outer, x, y, w, h);
	set_section_dimension(inner, 0, 0, w, h);
}
...
function set_section_oi_original_dimension(section, w, h)
{
	if (section == null)
		return;
	...
	set_section_original_dimension(outer, w, h);
	set_section_original_dimension(inner, w, h);
}
...
function reset_section_oi_to_original_dimension(section)
{
	if (section == null)
		return;
	...
	reset_section_to_original_dimension(outer);
	reset_section_to_original_dimension(inner);
}
...
function set_section_current_dimension(section)
{
	...
	if (is_outer(section) && section.is_sized)
		return;
		
	if (is_inner(section) && go(section).is_sized)
	{
		section.width = go(section).width;
		section.height = go(section).height;
		return;
	}
		
	// The outer section size is equal to that of the inner section. The bars and other things are positioned in children positioning
	if (is_outer(section))
	{
		if (is_root(section))
		{
	...
	// if this is an inner section with no children then its dimensions are set to the original dimensions
	if (is_inner(section))
	{
		if (is_root(section))
		{
		...
		// if the only child is the center section then reset the size to the original values
		if (is_center_the_only_child_and_visible(section))
		{
			reset_section_to_original_dimension(section);
			return;
		}
...
function set_section_children_current_position(section)
{
	...
	if (is_inner(section) && is_center(go(section)))
		return set_non_docked_children_current_position(section);
	...
	for (var idx=0; idx < section.child_sections.length;idx++)
	{
		...
		{
			// if this is the center child, i.e container for non docked grand children, 
			// then save the reference and fill up the remaining non docked space with it
			if (is_center(child_section))
			{
				the_center_child = child_section;
				continue;
			}
		...
		if (child_section.position_type == "dock" && !is_center(child_section))
		{
	...
	if (the_center_child != null)
		set_section_dimension(the_center_child, mx, my, mr-mx, mb-my);		
	
	// if there is no center section then the last child will occupy the remaining space.
	else if (the_last_child != null && the_last_child.position_type == "dock")
	{
...
function set_non_docked_children_current_position(section)
{
	...
	if (is_inner(section) && is_center(section))
	{
		var prevx = 0;
		var prevy = 0;
		var maxb = 0;
...
function check_set_on_overflow(section)
{
	...
	if (section.on_inner_overflow == "none")
		return;
	...
	{
		if (section.on_inner_overflow.match("scroll_bars"))
...
{
	var outer = go(section);
	set_max_zindex(outer);

	var inner = gi(section);
	set_max_zindex(inner);
}

...
// ================================================================================
function create_abstract_section(parent_section, id)
{
...
function create_section(parent_section, id, x, y, w, h, pos_type)
{
	...
	// create the outer
	var section = create_abstract_section(parent_section, id);
	...
	// ============================================
	return section; // from this the inner can be got, from which outer can be got again, from the inner, center (ndock) can be got.
}
...
function create_section_iframe(section, src)
{
	...
	var frame_handle = section.frame_handle;
  	if (section.is_frame)
  	{
  		...
  		{
  			frame_handle = g_create_frame("frame_" + section.id, src);
...
function contain_root_container(id, side)
{
    ...
    destroy_section_borders(groot_container);
	
	var moved = false;
	for (var idx1 = 0; idx1 < groot_container.child_sections.length; idx1++)
	{
	    var old_child = groot_container.child_sections[idx1];
	    remove_child_section(old_child);
		set_section_parent(old_child, container);
		idx1--;
...
//	
	...
	container.real_type = "root";
	return container;
}

function contain_section(section, id) {
    if (section == null || section.parent_section == null || section.parent_section.child_sections == null)
        return;
...
// ================================================================================
function add_section(parent_section, id, x, y, w, h, src, position_type, title_text)
{
	reset_section_is_sized(groot_container, false, true);
...
//		parent.real_type = "container";
	//
    if (!is_root(parent) && position_type == "dock")
        parent.real_type = "dock_container";
	...
	var section = null;
	if (position_type == "tab") {
	    section = create_section_and_add_tab(parent, id, x, y, w, h, "bottom", title_text); // by default do a bottom tab, then by add_property, change the tab position
	}
	else if (position_type == "tab_node") {
	    section = create_section_and_add_tab_node(parent, id, x, y, w, h, "bottom", title_text); // by default do a bottom tab, then by add_property, change the tab position
	}
	else {
...
function add_property(section, name, value)
{
	...
	else if (name == "tab_text") {
	    section.tab_text = value;
//	    if (tabc != null)
//	        tabc.tab_text = value;
	}
	else if (name == "tab_position")
	    section.tab_position = value; // that of the tab container
	else if (name == "tab_length")
	    section.tab_length = value;
	else if (name == "tab_breadth")
	    section.tab_breadth = value;
	else if (name == "tabs_overlap")
	    section.tabs_overlap = value;
	else if (name == "tab_length_fixed")
	    section.tab_length_fixed = value;
	else if (name == "tab_min_length")
	    section.tab_min_length = value;
	else if (name == "tabbar_padding")
	    section.tabbar_padding = value;
	else if (name == "tab_text") {
	    section.tab_text = value;
	}
...
/*
	...
	this.on_overflow = "none"; // none, hide, fit, resize, scroll_bars, scroll_arrows, reflow, resize_height, resize_width 
	this.on_outer_overflow = "none"; // none, repos_left, repos_top
*/

function go(section) { return section; }
function gi(section) { return section; }
function gco(section) { return section; }
function gci(section) { return section; }
function gcoo(section) { return section.parent_section; }
function gcoi(section) { return section.parent_section; }
function gpo(section) { return section.parent_section; }
function gpi(section) { return section.parent_section; }

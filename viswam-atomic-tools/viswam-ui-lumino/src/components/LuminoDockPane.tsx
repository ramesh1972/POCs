...
// Helper to clear all commands before (re)registering
function clearAllCommands() {
  // @ts-ignore: access private _commands for reset
  if (commands && (commands as any)._commands) {
    (commands as any)._commands.clear();
  }
  // Remove all key bindings
  if (commands && (commands as any)._keyBindings) {
    (commands as any)._keyBindings.length = 0;
  }
}
...
const LuminoDockPane: React.FC = () => {
  ...
  useEffect(() => {
    ...
    function main(): void {
      ...
      let s1 = new ContentWidget('Split1');
      let s2 = new ContentWidget('Split2');
      s1.node.style.backgroundColor = 'lightgray';
      s2.node.style.backgroundColor = 'lightgray';
      let splitPanel = new SplitPanel();
      splitPanel.id = 'split-panel';
      splitPanel.addWidget(s1);
      splitPanel.addWidget(s2);
      let s3 = new ContentWidget('Split3');
    // add splitpanel to s3
      
      s3.node.style.backgroundColor = 'lightgray';
      s3.node.appendChild(splitPanel.node);
      ...
      dock.id = 'dock';
      dock.addWidget(s3, { mode: 'split-bottom', ref: g1 });
      ...
      Widget.attach(bar, containerRef.current!);
      Widget.attach(main, containerRef.current!);
    }

    main();

    // Cleanup: remove all Lumino widgets on unmount
    return () => {

...
export class App {
    ...
    constructor(controlsElement?: HTMLElement, layoutElement?: HTMLElement) {
        // Use provided elements or fallback to querySelector for legacy usage
        const controls = controlsElement ?? document.querySelector('#controls') as HTMLElement;
        if (!controls) throw new Error('controlsElement not found');
        this._controlsElement = controls;

        const layout = layoutElement ?? document.querySelector('#layoutContainer') as HTMLElement;
        if (!layout) throw new Error('layoutContainerElement not found');

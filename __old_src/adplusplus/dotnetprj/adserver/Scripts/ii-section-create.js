...
function page_main() {
    bodyx.onclick = page_onclick;
    bodyx.onresize = resize_page;
    bodyx.onmousemove = body_mousemove;
...
function CSection()
{
	...
	this.title = "No Title";
	this.title_bar = null;
	this.title_bar_position = "none";
	this.title_bar_thickness = 16;

	// internally used objects/vars
	this.tabs = null;
	this.tab_bar = null;
}
...
function create_section_iframe(section, src)
{
	...
	var frame_handle = section.frame_handle;
  	if (section.is_frame)
  	{
        ...
        {
            if (section.outer_section_ref.url_delay_load == false)
                frame_handle = g_create_inner_frame("frame_" + section.id, src);
            else
                frame_handle = g_create_inner_frame("frame_" + section.id, "");
...
{
    sss(vsection, "backgroundColor", vsection.outer_section_ref.bgcolor);
	sss(vsection, "overflow", "auto");
	sss(vsection, "borderStyle", "none");

	if (vsection.outer_section_ref.bgimage != null || vsection.outer_section_ref.bgimage != undefined) {
	    sss(vsection, "backgroundImage", vsection.outer_section_ref.bgimage);
	    sss(vsection, "backgroundSize", "100%");
	}
}
...
function set_iframe_default_styles(section)
{
	...
	if (frame_handle != null)
	{
	    g_ss(frame_handle, "borderWidth", 0);
		g_ss(frame_handle, "borderStyle", "none");
...
function add_property(section, name, value)
{
	...
	if (name == "real_type")
	    ...
	    outer.title_bar_position = value;
	    outer.title_bar = create_section_bar(outer, value);
	    return outer.title_bar;
	}
	else if (name == "title_font_size") {
	    outer.title_font_size = value;
	}
	else if (name == "title_bar_thickness") {
	    inner.title_bar_thickness = value;
	    outer.title_bar_thickness = value;
	}
	else if (name == "title") {
	    outer.title = value;
	    layout_bar_text(outer, value);
	}
	else if (name == "url_delay_load")
	    outer.url_delay_load = value;
	else if (name == "url")
	    ...
	    outer.bgimage = value;
	    set_inner_default_style(outer.inner_section_ref);
	}
	...
	else if (name == "tab_bar_visible" && tabc != null)
	    tabc.tab_bar_visible = value;
	else if (name == "text" && tabc != null)
	    set_tab_text(section.tab, value);
	else if (name == "tab_length")
	    section.tab.tab_length = value;
	else if (name == "tab_bgcolor")
	    section.bgcolor = section.tab.tab_bgcolor = value;
	else if (name == "tab_fgcolor")
	    section.fgcolor = section.tab.tab_fgcolor = value;
	else if (name == "tab_is_active") {
	    section.tab.tab_is_active = value;
	    set_tab_is_active(section.tab);
	}

...
{	
	...
	return false;
 	...
 	{
			document.frmChargeSelection.action="ubnkviewRes.asp";
...
{	
	...
	<tr class="tdbgdark">
	   <td valign=center colspan=2 class="whiteboldtext1">Select the criteria for  View :</td>
	</tr>
	
	<%elseif Request.QueryString("Mode")="D" then%>
	<tr class="tdbgdark">
	   <td valign=center colspan=2 class="whiteboldtext1">Select the criteria for Delete :</td>
	</tr>
	<%end if%>
   <input type=hidden name=txthidMode value=<%=Request.QueryString("Mode")%>>		
	...
	if not lObjRsBnkGrp.EOF and trim(Request.Form("txtBnkCode"))="" then
	 ...
	 <td colspan=3 valign="center" align=middle class="tdbglight">
   		<input type="Button" name="sbtAdd"   value="View"   onclick ="btnClick()">&nbsp;&nbsp;&nbsp;
   		<input type="reset" name="sbtCancel" value="Cancel" onclick ="btnClick1()">
	</td>
	</tr>
	</table>

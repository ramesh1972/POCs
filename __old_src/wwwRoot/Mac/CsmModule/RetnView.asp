...
%>
	...
	function btnClick(fValue)
	{
		...
		if (i==1) {
			document.frmRetnSelection.hidActionvalue.value =1;
			if (document.frmRetnSelection.OptUserId.value==""){
				alert("Select User Id & Name");
				return false;}
			else{
				document.frmRetnSelection.action="RetnViewRes.asp";
	...
	<tr class="tdbgdark">
	   <td valign=center colspan=2 class="whiteboldtext1">Select the criteria to View Retention Master Details</td>
	</tr>
	<%elseif Request.QueryString("Mode")="D" then%>
	<tr class="tdbgdark">
	   <td valign=center colspan=2 class="whiteboldtext1">Select the criteria to Delete Retention Master Details</td>
	</tr>
	<%end if%>
   <input type=hidden name=txthidMode value=<%=Request.QueryString("Mode")%>>		
	 <TR><TD class=tdbglight width=25% height="20"><font class=blacktext>User Id & Name</font></TD>

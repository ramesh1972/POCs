...
function btnClick(fValue)
	...
	var i=fValue;
	if (i==1) {
		document.frmBankView.action="BankDelRes.asp";
...
<%
	...
	set lObjBnkUpdate	= server.CreateObject("Mac.BankMasterMgr")
	lResult	=	lObjBnkUpdate.DoBnkMasBrowse(lBankCode,lBranchCode,lBranchname,lAddress1,lAddress2,lAddress3)
	if lResult=0 then
%>	
...
<table width="55%" border="1"  cellpadding=2 cellspacing=1 align=center height="146">
    ...
    <tr>
      ...
      <td WIDTH="367" CLASS=tdbglight height="25"><%=lBranchname%>
    </tr>
          
    <tr>
      <td class=tdbglight width=30% height="25"><font class=blacktext>Branch Address1</font> </td>
      <td WIDTH="367" CLASS=tdbglight height="25"><%=lAddress1%>
    </tr>
    
    <tr>
      <td class=tdbglight width=30% height="25"><font class=blacktext>Branch Address2</font> </td>
      <td WIDTH="367" CLASS=tdbglight height="25"><%=lAddress2%>
    </tr>
    
    <tr>
      <td class=tdbglight width=30% height="25"><font class=blacktext>Branch Address3</font> </td>
      <td WIDTH="367" CLASS=tdbglight height="25"><%=lAddress3%>
    </tr>
    
    <tr>
		<td class=tdbglight align="Center" colspan=2>
		<input type="reset" align="center" name="sbtCancel" value="Cancel"  onclick ="return btnClick(2);">
		</td>
	</tr>	
...
<%
 ...
 <input type=hidden name=txthidComgCode value=<%=trim(Request.Form("optBnkCode"))%>>
 <input type=hidden name=txthidContCode value=<%=trim(Request.Form("optBrnCode"))%>>
<%end if%>
</table>			
<%else%>
<center>

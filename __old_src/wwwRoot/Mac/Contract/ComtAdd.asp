...
%>
	...
	if (document.frmComtSelection.OptComgCode.value==""){
		...
		if(document.frmComtSelection.txtComtType.value==""){
			alert("Enter Commodity Type")
			document.frmComtSelection.txtComtType.focus()
			return false;}
		...
		if(document.frmComtSelection.txtMaop.value<0){
			...
			document.frmComtSelection.action="ComtAddRes.asp";
	...
	<table width="65%" border="1"  cellpadding=2 cellspacing=1 align=center height="146">
	<tr class="tdbgdark">
	   <td valign=center colspan=2 class="whiteboldtext1">Commodity Type Master</td>
		
	</tr>
		
	 <TR><TD class=tdbglight width=40% height="20"><font class=blacktext>Commodity Group Code</font></TD>
	...
	if not lobjRsComgGrp.EOF and trim(Request.Form("txtComgCode"))="" then
	 ...
	 <tr>
		...
		<td WIDTH="367" CLASS=tdbglight height="25">
		<input type=text name=txtComtType value="">
     </tr>

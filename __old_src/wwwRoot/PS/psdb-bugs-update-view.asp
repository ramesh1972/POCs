		...
		do while lRecordsShown < cint(lPageSize) and not rst.EOF
			...
			If rst("BR_Identifier") <> "" Then
				if lHEmpAdmin = false And lHEmpProjectLead=false And lHEmpDeveloper = "N" And lHEmpTester = "Y" Then
					Response.Write("<A href=psdb-bugs-tester-update.asp?Identifier=" & rst("BR_Identifier") & ">" & rst("BR_Identifier") & "</A>")
				else 
			...
			lName = lSBObj.GetModuleName(eval(trim(rst("M_ModuleID"))), eval(trim(rst("M_ModuleID"))))
			Response.Write("<TD>")
			If lName <> "" Then

...
<% 'Get all required data from the database
	...
	if lGroupRs.RecordCount > 0 Then
		...
		Response.Write("<TD align=center><font class=whiteboldtext>Project Bugs Database</font></TD>")				
		Response.Write("</TR>")			
		...
		while Not lGroupRs.EOF
			...
			If trim(lGroupRs("G_GroupID")) <> "" Then
				if (lUserType = "A") Then
					Response.Write("<A href=psdb-bugs-database-view.asp?ProjectID=" & trim(lGroupRs("G_GroupID")) & ">" & trim(lGroupRs("G_GroupID")) & "</A>")
				Else
			...
			Dim lDatabaseStr
			lDatabaseStr = ""
			Set lDatabaseRs = lBugsObj.GetBugsDatabase(eval(trim(lGroupRs("G_GroupID"))), 0)
			while not lDatabaseRs.EOF 
				lDatabaseStr = lDatabaseStr & trim(lDatabaseRs("DB_Name")) & "<BR>"
				lDatabaseRs.MoveNext 
			wend

			Response.Write("<TD valign='top'>")
			If lDatabaseStr <> "" Then
				Response.Write(lDatabaseStr)
			Else
				Response.Write("&nbsp")
			End If
			Response.Write("</TD>")
	...
	<%
		...
		<TR>
			<TD><font class="whiteboldtext">Project Bugs Database details not available</font></TD>
		</TR>
		</TABLE>

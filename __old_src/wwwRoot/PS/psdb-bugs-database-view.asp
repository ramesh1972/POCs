	...
	if lDatabaseRs.RecordCount > 0 Then
		...
		while Not lDatabaseRs.EOF
			...
			If trim(lDatabaseRs("G_GroupID")) <> "" Then
				Response.Write("<A href=psdb-bugs-database-update.asp?ProjectID=" & lProjectID & "&BugsDatabaseID=" & trim(lDatabaseRs("BD_DatabaseID")) & ">" & trim(lDatabaseRs("BD_DatabaseID")) & "</A>")
			Else
				Response.Write("&nbsp")
			End If
			Response.Write("</TD>")

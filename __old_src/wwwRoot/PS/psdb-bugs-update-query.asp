...
%>
<% Call DisplayTitle() %><BR>
<FORM ACTION = "psdb-bugs-update-view.asp" method="post" name="frmViewQuery" onsubmit="return validateDate();CheckEndDate();">
...
<%
	Dim lOnlyTester 
	lOnlyTester = false
	if lHEmpAdmin = false And lHEmpProjectLead=false And lHEmpDeveloper = "N" And lHEmpTester = "Y" Then		
		lOnlyTester = true
		lSelectStr = "<SELECT name='sBugsReportedBy' disabled><Option Value='0'>All</Option>"
	else
	...
	If lRs.RecordCount > 0 Then
		...
		While not lRs.EOF
			if lOnlyTester Then		
				if eval(lEmpID) = eval(lRs("U_Employee_ID")) Then
					lSelectStr = lSelectStr & "<Option Value='" & lRs("U_Employee_ID") & "' selected>" & lRs("U_Name") & "</Option>"
				else
...
<TR>
	<TD width=50% align='right'><INPUT type='button' Value='<< Back' style='BACKGROUND-COLOR: aqua' onclick='history.back()' id='button'1 name='button'1></TD>
	<TD width=50% align="left"><INPUT type="submit" Value="View Bugs" style='BACKGROUND-COLOR: aqua' id=submit1 name=submit1></TD>
</TR>
</TABLE>

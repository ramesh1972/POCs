function LoadPanelControl(control_id, control_row_ui) 
{
	...
	var next_control = GetNextSiblingNode(control_id, "");
	...
	if (table_closed) {
		...
		var parent_div = GetChildNodeValue(control_parent, "PanelID");
	...
	var controls_sub = GetNodeChildrenWithType(control_id, "");
	var prev_row = 0;
	...
	for (var idx=0; idx<controls_sub.length; idx++) 
	{
		...
		if (elem_type == "Control") {
			...
			if (row != prev_row)
				LoadPanelControl(controls_sub[idx], "");
				
			prev_row = row;
		}
		else if(elem_type == "Element")
			LoadControlElement(controls_sub[idx]);
	}
...
function LoadControlElement(element_id) {
	...
	// if diff from the current row then close the table tag
	var next_element = GetNextSiblingNode(element_id, "");
	...
	// get all children that are TreeNodes in order of cells, row-col
	var elements_sub = GetNodeChildrenWithType(element_id, "");
	var prev_row = 0;
	...
	for (var idx=0; idx<elements_sub.length; idx++) 
	{
		...
		if (row != prev_row) {
			LoadControlElement(elements_sub[idx]);
		}

...
<% 'Get the userid and projectid
	...
	lEmpIDCombo = Request.Form("sUsers")
...
<BR>
    <table width="47%" border="1"  cellpadding="0" cellspacing="0" align=center STYLE="border-collapse:collapse">
      <tr class="tdbgdark"> 
        <td valign=center colspan=2><span class="whiteboldtext1">Generating Weekly Progress Report...
          </span></td>
      </tr>
     </table>
	<table width="47%" border="1"  cellpadding="0" cellspacing="0" align=center STYLE="border-collapse:collapse">
<%	' Generate the report
	...
	lFileName = lTSObj.GetProgressFileName (eval(lEmpIDCombo), lMondayDate)
	lFileFullName = "C:\inetpub\wwwroot\timesheet\reports\users\" & lFileName
	
	lCreated = lTSObj.GenPRForUser (lFileFullName, eval(lEmpIDCombo), eval(lProjectID), lMondayDate)

...
function CreateNewFilledSectDatesRow(piSectNo, piTaskDesc, piTaskBaseStart, piTaskBaseEnd, piTaskActStart, piTaskActEnd) {
	...
	if (piSectNo == 1) {
		...
		lCount = gSect1RowCount;
	}
	if (piSectNo == 2) {
		gSect2RowCount = gSect2RowCount + 1;	
		lCount = gSect2RowCount;
	}
	if (piSectNo == 3) {
		gSect3RowCount = gSect3RowCount + 1;	
		lCount = gSect3RowCount;
	}
	...
	oCell.innerHTML = "<INPUT type='text' size='30' maxlength=256 name='ebSect" + piSectNo + "TaskDesc" + lCount + "' value='" + piTaskDesc + "'>";  

	oCell = oRow.insertCell();
	oCell.innerHTML = "<INPUT type='text' size='10' maxlength=10 name='ebSect" + piSectNo + "TaskBaseStart" + lCount + "' value='" + piTaskBaseStart + "' onkeypress='ValidateInteger();'>";    

	oCell = oRow.insertCell();
	oCell.innerHTML = "<INPUT type='text' size='10' maxlength=10 name='ebSect" + piSectNo + "TaskBaseEnd" + lCount + "' value='" + piTaskBaseEnd + "' onkeypress='ValidateInteger();'>";    

	oCell = oRow.insertCell();
	oCell.innerHTML = "<INPUT type='text' size='10' maxlength=10 name='ebSect" + piSectNo + "TaskActStart" + lCount + "' value='" + piTaskActStart + "' onkeypress='ValidateInteger();'>";    

	oCell = oRow.insertCell();
	oCell.innerHTML = "<INPUT type='text' size='10' maxlength=10 name='ebSect" + piSectNo + "TaskActEnd" + lCount + "' value='" + piTaskActEnd + "' onkeypress='ValidateInteger();'>";    
}
...
function CreateNewFilledSect45Row(piSect, piTaskDesc, piStatus, piRemarks) {
	...
	if (piSect == 4) {
		...
		lCount = gSect4RowCount;
	}
	if (piSect == 5) {
		gSect5RowCount = gSect5RowCount + 1;
		lCount = gSect5RowCount;
	}
	...
	// Insert rows and cells into the body.
	oCell = oRow.insertCell();
	oCell.innerHTML = "<INPUT type='text' size='33' maxlength=256 name='ebSect" + piSect + "Issue" + lCount + "' value='" + piTaskDesc + "'>";  

	oCell = oRow.insertCell();
	oCell.innerHTML = piStatus;    

	oCell = oRow.insertCell();
	oCell.innerHTML = "<INPUT type='text' size='33' maxlength=256 name='ebSect" + piSect + "IssueRemarks" + lCount + "' value='" + piRemarks + "'>";  
}
...
function CreateNewFilledSectNotesRow(piSect, piNote) {
	...
	if (piSect == 6) {
		...
		lCount = "Notes" + gSect6NotesRowCount;
	}
	if (piSect == 7) {
		gSect6FlagsRowCount = gSect6FlagsRowCount + 1;
		lCount = "Flags" + gSect6FlagsRowCount;
	}
	...
	// Insert rows and cells into the body.
	oCell = oRow.insertCell();
	oCell.innerHTML = "<INPUT type='text' size='64' maxlength=256 name='ebSect6" + lCount + "' value='" + piNote + "'>";  
}
...
function CreateSection1NewRow() {
	...
	gSect1RowCount = eval(gSect1RowCount) + 1;
	...
	document.frmUpdateProgressReport.hSect1RowCount.value = gSect1RowCount; 	
}
...
function CreateSection2NewRow() {
	...
	// increment the global variab;e
	gSect2RowCount = eval(gSect2RowCount) + 1;
	...
	// Update the hRowCount hidden variable
	document.frmUpdateProgressReport.hSect2RowCount.value = gSect2RowCount; 	
}
...
function CreateSection3NewRow() {
	...
	// increment the global variab;e
	gSect3RowCount = eval(gSect3RowCount) + 1;
	...
	// Update the hRowCount hidden variable
	document.frmUpdateProgressReport.hSect3RowCount.value = gSect3RowCount; 	
}
...
function CreateSection4NewRow() {
	...
	// increment the global variab;e
	gSect4RowCount = eval(gSect4RowCount) + 1;
	...
	// Update the hRowCount hidden variable
	document.frmUpdateProgressReport.hSect4RowCount.value = gSect4RowCount; 	
}
function CreateSection5NewRow() {
	...
	// increment the global variab;e
	gSect5RowCount = eval(gSect5RowCount) + 1;
	...
	// Update the hRowCount hidden variable
	document.frmUpdateProgressReport.hSect5RowCount.value = gSect5RowCount; 	
}
...
function CreateSection6NotesNewRow() {
	...
	// increment the global variab;e
	gSect6NotesRowCount = eval(gSect6NotesRowCount) + 1;
	...
	// Update the hRowCount hidden variable
	document.frmUpdateProgressReport.hSect6NotesRowCount.value = gSect6NotesRowCount; 	
}
...
function CreateSection6FlagsNewRow() {
	...
	// increment the global variab;e
	gSect6FlagsRowCount = eval(gSect6FlagsRowCount) + 1;
	...
	// Update the hRowCount hidden variable
	document.frmUpdateProgressReport.hSect6FlagsRowCount.value = gSect6FlagsRowCount; 	
}
...
<%  ' Get the record set for the progress report
	...
	Set lProgressReportRs = lTSObj.GetPRSectionDates(eval(lEmpID), eval(lProjectID), lMondayDate)
...
<%
	...
	Set lProgressReportRs = lTSObj.GetPRSection45 (eval(lEmpID), eval(lProjectID), lMondayDate)
	if lProgressReportRs.RecordCount > 0 Then
		lProgressReportRs.MoveFirst 
		...
		while not lProgressReportRs.EOF
			if (eval(trim(lProgressReportRs("WPR_SectionNo"))) = 4) Then
				...
				lStatusSelectStr = "<Select name='sSect4IssueStatus" & gSect4RowCount & "'>" 
...
<%
			...
			if (eval(trim(lProgressReportRs("WPR_SectionNo"))) = 5) Then
				...
				lStatusSelectStr = "<Select name='sSect5IssueStatus" & gSect5RowCount & "'>" 
				if trim(lProgressReportRs("WPR_TaskStatus")) = "Open" Then
					lStatusSelectStr = lStatusSelectStr & "<OPTION Value='Open' selected>Open</OPTION>" & "<OPTION Value='Closed'>Closed</OPTION>"
				else 
					lStatusSelectStr = lStatusSelectStr & "<OPTION Value='Open'>Open</OPTION>" & "<OPTION Value='Closed' selected>Closed</OPTION>"
...
<%
	...
	Set lProgressReportRs = lTSObj.GetPRSectionNotes (eval(lEmpID), eval(lProjectID), lMondayDate)
	if lProgressReportRs.RecordCount > 0 Then
		lProgressReportRs.MoveFirst 
...
<TR>
	<TD align="center"><INPUT type="button" Value="New Entry" onclick="CreateSection1NewRow();" style="BACKGROUND-COLOR: aqua" id=button1 name=button1></TD>
</TR>
</TABLE>
...
<TABLE width="75%" align="center">
<TR>
	<TD align="center"><INPUT type="button" Value="New Entry" onclick="CreateSection3NewRow();" style="BACKGROUND-COLOR: aqua" id=button3 name=button3></TD>
</TR>
</TABLE>
<% 
	Dim lIdx1
...
<TR>
	<TD align="middle">
	<INPUT type="submit" Value="Save Progress Report" style="BACKGROUND-COLOR: aqua" id=submit1 name=submit1>
...
<% ' Create empty bodies for empty sections
	If Not lSect1DataExists Then
%>
<script language="javascript">
...
%>
<% ' Create empty bodies for empty sections
	If Not lSect6NotesDataExists Then
%>
<script language="javascript">
...
%>
<% ' Create empty bodies for empty sections
	If Not lSect6FlagsDataExists Then
%>
<script language="javascript">

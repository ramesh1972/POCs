...
function create_border_section(section, pos)
{
	...
	var bsection = create_abstract_section(section.parent_section, id, "");
...
function undo_section_border_dimensions(section, deep) // dunit
{
	if (section == null)
		return;
	
	// only sections of these 2 position types share the border, so the dims have to be undone only in these cases
	if (section.is_sizable && (section.position_type == "dock" || section.position_type == "stack"))
	{
		var bt = get_parent(section).border_thickness;
...
function remove_overlapped_borders(container)
{
	...
	for (var idx=0; idx < container.child_sections.length; idx++)
	{
		...
		var bt = get_parent(border.base).border_thickness;
		for (var idx1=0; idx1 < container.child_sections.length; idx1++)
		{
			...
			var bt1 = get_parent(border1.base).border_thickness;
...
function get_border_from_event(evnt)
{
	...
	var id = "section_" + obj.id.substring(8);
		
	var border = get_section_from_id(groot_container, id);
	return border;
}

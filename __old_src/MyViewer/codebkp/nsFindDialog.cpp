...
//---------------------------------------------------------------
void nsFindDialog::Initialize(nsBrowserWindow * aWindow) 
{
  ...
  if (nsnull != doc) {
    ...
    aWindow->AddEventListener(mDwnRB);
	  ...
	  nsCOMPtr<nsIDOMEventTarget> evtTarget(do_QueryInterface(mFindBtn));
  if (evtTarget)
    evtTarget->AddEventListener(NS_LITERAL_STRING("click"), (nsIDOMMouseListener*) aWindow, PR_FALSE);
    SetChecked(mMatchCaseCB, PR_TRUE);
    NS_RELEASE(doc);
  }
}

//-----------------------------------------------------------------
void nsFindDialog::MouseClick(nsIDOMEvent* aMouseEvent, nsBrowserWindow * aWindow, PRBool &aStatus)
{
...
//-----------------------------------------------------------------
void nsFindDialog::Destroy(nsBrowserWindow * aWindow)
{
...
{
  mWindow->SetVisibility(PR_FALSE);
}

...
//---------------------------------------------------------------
void nsBaseDialog::Initialize(nsBrowserWindow * aWindow) 
{
  mWindow = aWindow;
  NS_ADDREF(mWindow);
...
//-----------------------------------------------------------------
void nsBaseDialog::Destroy(nsBrowserWindow * aWindow)
{
...
//-----------------------------------------------------------------
void nsBaseDialog::MouseClick(nsIDOMEvent* aMouseEvent, nsBrowserWindow * aWindow, PRBool &aStatus)
{
...
nsBaseDialog::SetChecked(nsIDOMElement * aNode, PRBool aValue)
{
  nsIDOMHTMLInputElement * element;
  if (NS_OK == aNode->QueryInterface(kIDOMHTMLInputElementIID, (void**) &element)) {
		nsString s(NS_LITERAL_STRING("HAHAHA"));

		nsIDOMNode* aNextSibling;
		element->GetNextSibling(&aNextSibling);
	  aNextSibling->SetNodeValue(s);

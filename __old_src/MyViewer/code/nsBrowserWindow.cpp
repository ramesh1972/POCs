...
static NS_DEFINE_IID(kIDOMHTMLInputIID, NS_IDOMHTMLINPUTELEMENT_IID);
...
static nsEventStatus PR_CALLBACK
HandleParentWindowEvent(nsGUIEvent *aEvent)
{ 
  nsEventStatus result = nsEventStatus_eIgnore;
  switch (aEvent->message) {
      case NS_PAINT:{
...
#define WALLET_EDITOR_URL "file:///y|/walleted.html"
  ...
  case VIEWER_FILE_OPEN:
	...
	case ID_FILE_CLICK:
	{
		...
		nsIDOMElement* a;
		nsAutoString id(NS_LITERAL_STRING("clickme"));
		doc->GetElementById(id, &a);

		nsCOMPtr<nsIDOMDocumentEvent> docEvent(do_QueryInterface(doc));
    nsCOMPtr<nsIDOMEvent> event;
    docEvent->CreateEvent(NS_LITERAL_STRING("MouseEvents"), getter_AddRefs(event));
		event->InitEvent(NS_LITERAL_STRING("click"), PR_TRUE, PR_TRUE);

    nsCOMPtr<nsIPrivateDOMEvent> privateEvent(do_QueryInterface(event));
    privateEvent->SetTrusted(PR_TRUE);

    PRBool noDefault;
    nsCOMPtr<nsIDOMEventTarget> targ(do_QueryInterface(a));
...
{
  if (mFindDialog != nsnull) 
  {
	mFindDialog->AddWindowListener(mFind); 
    mFindDialog->SetVisibility(PR_TRUE);
    return;
  }
  ...
  if (nsnull != domDoc) 
  {
	nsString printHTML(NS_LITERAL_STRING("resource:/res/samples/find.html"));
	nsRect rect(0, 0, 375, 310);
	nsString title(NS_LITERAL_STRING("Find"));

	//	nsIFactory* ifac;
	//	NS_NewXPBaseWindowFactory(&ifac);
	//  nsresult rv = ifac->CreateInstance(nsnull,
	//                                                 kIXPBaseWindowIID,

	mFindDialog = new nsNativeBrowserWindow();
	nsresult rv = NS_OK;
	if (rv == NS_OK) 
	{
		mFindDialog->Init(mAppShell, rect, PRUint32(~0), PR_FALSE);
		mFindDialog->SetVisibility(PR_TRUE);
		mFind = new nsFindDialog(this); // ref counts domDoc
		mFindDialog->AddWindowListener(mFind); 
		nsWebCrawler* Crawler = new nsWebCrawler(mApp);
		Crawler->SetBrowserWindow(mFindDialog);
		mFindDialog->SetWebCrawler(Crawler);

		nsWeakPtr weakling(
		do_GetWeakReference((nsIWebProgressListener*)Crawler));
		mFindDialog->mWebBrowser->AddWebBrowserListener(weakling, NS_GET_IID(nsIWebProgressListener));

		//mFindDialog->mWebBrowserChrome->BrowserWindow(this);
		Crawler->AddURL(printHTML);
		Crawler->Start();
	}
    NS_RELEASE(domDoc);
  }
                      ...
                      PRBool aAllowPlugins)
{
  ...
  if (nsIWebBrowserChrome::CHROME_MENUBAR & aChromeMask) {
    ...
    mWindow->GetClientBounds(r);    r.x = r.y = 0;
  }
...
void nsBrowserWindow::DoTableInspector()
{
  if (mTableInspectorDialog) {
    mTableInspectorDialog->SetVisibility(PR_TRUE);
    return;
  }
  nsIDOMDocument* domDoc = GetDOMDocument(mDocShell);
  ...
  if (nsnull != domDoc) {
    ...
    nsBrowserWindow * xpWin = nsnull;
    nsresult rv = NS_OK ; //nsComponentManager::CreateInstance(kXPBaseWindowCID, nsnull,
                            //                         kIXPBaseWindowIID,
                              //                       (void**) &xpWin);
    /*if (rv == NS_OK) {
      xpWin->Init(mAppShell, rect, PRUint32(~0), PR_FALSE);
      xpWin->SetVisible(PR_TRUE);
      if (NS_OK == xpWin->GetInterface(kIXPBaseWindowIID, (void**) &mTableInspectorDialog)) {
...
NS_IMETHODIMP nsBrowserWindow::EnsureWebBrowserChrome()
{
   ...
   mWebBrowserChrome1 = new nsWebBrowserChrome();
   NS_ENSURE_TRUE(mWebBrowserChrome, NS_ERROR_OUT_OF_MEMORY);
   NS_ENSURE_TRUE(mWebBrowserChrome1, NS_ERROR_OUT_OF_MEMORY);

   NS_ADDREF(mWebBrowserChrome);
   NS_ADDREF(mWebBrowserChrome1);
   mWebBrowserChrome->BrowserWindow(this);
   mWebBrowserChrome1->BrowserWindow(this);
...
{ 
  ...
  nsSizeEvent* sizeEvent;
    ...
    case NS_PAINT:{
      ...
      cx->SetColor(NS_RGB(255, 100, 100));
      cx->FillRect(0, 0, bounds.width, bounds.height);
	        result = nsEventStatus_eConsumeDoDefault;
		break;
				  }
...
NS_IMETHODIMP nsBrowserWindow::EndLoadURL(nsIWebShell* aShell, const PRUnichar* aURL, nsresult aStatus)
{
...
void nsBrowserWindow::AddMOEvents() {
  if (nsnull != mDocShell) {
    
    nsIPresShell* shell = GetPresShellFor(mDocShell);
    if (nsnull != shell) {
      nsCOMPtr<nsIDocument> doc;
      shell->GetDocument(getter_AddRefs(doc));
      if (doc) {
...
void nsBrowserWindow::RecurseDOM(nsIContent* r) 
{
  PRInt32 index;
  nsAutoString buf;
  
  if (r != nsnull && r->GetNodeInfo() != nsnull)  {
	r->GetNodeInfo()->GetQualifiedName(buf);
	char* s = new char[100];
	s = buf.ToCString(s, 100);
  
		

  nsIDOMHTMLElement* dom = (nsIDOMHTMLElement*) r;
//  PRUint16 t;
  //dom->GetNodeType(&t);
//  PRUint16 x = 14;
  //if (t== nsIDOMNode::ELEMENT_NODE) {
	//  nsIDOMHTMLInputElement* x;
	  //dom->QueryInterface(kIDOMHTMLInputIID, (void**) &x);
	  //x->SetReadOnly(PR_FALSE);
  //}
    nsIDOMEventReceiver * receiver;

     NS_PRECONDITION(nsnull != dom, "adding event listener to null node");

  if (NS_OK == dom->QueryInterface(kIDOMEventReceiverIID, (void**) &receiver)) {
    receiver->AddEventListenerByIID((nsIDOMKeyListener*)this, kIDOMKeyListenerIID);
  }
    NS_RELEASE(receiver);
    

  PRInt32 kids = r->GetChildCount();
  for (index = 0; index < kids; index++) {
    nsIContent *kid = r->GetChildAt(index);
	RecurseDOM(kid);
  }
  }
}
...
NS_IMETHODIMP nsBrowserWindow::AddEventListener(nsIDOMNode * aNode)
{
  nsIDOMEventReceiver * receiver;
...
//-----------------------------------------------------------------
NS_IMETHODIMP nsBrowserWindow::RemoveEventListener(nsIDOMNode * aNode)
{
  nsIDOMEventReceiver * receiver;
  ...
  if (NS_OK == aNode->QueryInterface(kIDOMEventReceiverIID, (void**) &receiver)) {
    receiver->RemoveEventListenerByIID((nsIDOMMouseListener*)this, kIDOMMouseListenerIID);
    NS_RELEASE(receiver);
    return NS_OK;
  }
  return NS_ERROR_FAILURE;
...
//-----------------------------------------------------------------
NS_IMETHODIMP nsBrowserWindow::AddWindowListener(nsIWindowListener * aWindowListener)
{
  if (nsnull != mWindowListener) {
    return NS_ERROR_FAILURE;
  }
...
nsresult nsBrowserWindow::HandleEvent(nsIDOMEvent* aEvent)
{
	nsString s(NS_LITERAL_STRING(""));
	aEvent->GetType(s);

	char* buff = new char[12];
	buff = s.ToCString(buff, 12);

	if (s.EqualsIgnoreCase("click") ==0)
			MouseClick(aEvent);
	else if (s.EqualsIgnoreCase("mouseover") == 0)
...
//-----------------------------------------------------------------
nsresult nsBrowserWindow::MouseUp(nsIDOMEvent* aMouseEvent)
{
  return NS_OK;
}

//-----------------------------------------------------------------
nsresult nsBrowserWindow::MouseDown(nsIDOMEvent* aMouseEvent)
{
  return NS_OK;
}

//-----------------------------------------------------------------
nsresult nsBrowserWindow::MouseClick(nsIDOMEvent* aMouseEvent)
{
    nsCOMPtr<nsIDOMEventTarget> target;
    aMouseEvent->GetTarget(getter_AddRefs(target));
	nsCOMPtr<nsIDOMElement> node(do_QueryInterface(target));
	nsString s(NS_LITERAL_STRING("Ramesh"));
    if (target) {
...
PRUint16 nodeType;
  ...
  if (nodeType == nsIDOMNode::ELEMENT_NODE) {
	  nsCOMPtr<nsIDOMHTMLInputElement> g(do_QueryInterface(node));
		  PRBool g1;
	  if (g) {
  ...
  if (nodeType == nsIDOMNode::TEXT_NODE ||
	...
	nsString s1(NS_LITERAL_STRING("bgcolor"));
	nsString s2(NS_LITERAL_STRING("red"));
	node->SetAttribute(s1,s2);
	...
	//nsIDOMNode* node_new;
	//body->AppendChild(node, &node_new);
...
//-----------------------------------------------------------------
nsresult nsBrowserWindow::MouseDblClick(nsIDOMEvent* aMouseEvent)
{
  return NS_OK;
}

//-----------------------------------------------------------------
nsresult nsBrowserWindow::MouseOver(nsIDOMEvent* aMouseEvent)
{
  return NS_OK;
}

//-----------------------------------------------------------------
nsresult nsBrowserWindow::MouseOut(nsIDOMEvent* aMouseEvent)
{
  return NS_OK;
}
NS_IMETHODIMP nsBrowserWindow::GetDocument(nsIDOMHTMLDocument *& aDocument)
{

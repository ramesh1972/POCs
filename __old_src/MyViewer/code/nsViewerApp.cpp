...
nsViewerApp::nsViewerApp()
{
  ...
  mStartURL.AssignWithConversion(text ? text : "http://www.csfb.com");
...
nsViewerApp::OpenWindow()
{
  ...
  if (!bwCurrent) {
    ...
    bw->SetWebCrawler(mCrawler);
	  ...
	  nsWeakPtr weakling(
      do_GetWeakReference((nsIWebProgressListener*)mCrawler));
  bw->mWebBrowser->AddWebBrowserListener(weakling, NS_GET_IID(nsIWebProgressListener));

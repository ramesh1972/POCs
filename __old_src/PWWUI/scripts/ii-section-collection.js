...
function get_section_at_xy(s_section, x, y)
{
	if (s_section == null)
		return null;
		...
		gst_s(s_section) + gsh(s_section) >= y)
	{
		...
		for (var idx=0;idx < s_section.child_sections.length;idx++)
		{
			...
			if (child.is_visible && !is_bar(child) && !is_paw(child) && !is_border(child))
			{
				...
				if (c_section != null && is_outer(c_section))
					return c_section;
			}
		}
...
function get_section_of_type_at_xy(s_section, type, x, y)
{
	if (s_section == null)
		return null;
		...
		gst_s(s_section) + gsh(s_section) >= y)
	{
		...
		if (is_outer(s_section) && s_section.real_type == type)
			return s_section;
		else
			return null;
	}

...
bool CRuleCreator::CreateRuleNode_AddContent(nsIDOMElement* p_dom_node,nsIDOMElement* p_rule_node)
{
	// get the leaf node type
	nsIDOMElement* p_leaf_node;
	LeafNodeType type = GetLeafNodeType(p_dom_node, &p_leaf_node);

	if (type == LNT_Text)
	{
		nsAutoString node_value = _rules_doc->GetNodeValue((nsIDOMElement*) p_leaf_node);
		//_ILOG(node_value);
		
		nsIDOMText* text = _rules_doc->CreateTextNode(p_rule_node, node_value);
		NS_RELEASE(text);
		NS_RELEASE(p_leaf_node);
		return true;
	}
	...
	if (type == LNT_Image)
	{
		...
		nsIImage* pimg;
		GetImageFromDOMNode((nsIDOMNode*)p_dom_node, &pimg);
		...
		nsIDOMNode* text =_rules_doc->CreateNodeWithText(p_rule_node, MozStr("BytesPix"), pimg->GetBytesPix());
		NS_RELEASE(text);
		text =_rules_doc->CreateNodeWithText(p_rule_node, MozStr("IsRowOrderTopToBottom"), pimg->GetIsRowOrderTopToBottom());
		NS_RELEASE(text);
		text =_rules_doc->CreateNodeWithText(p_rule_node, MozStr("PixWidth"), pimg->GetWidth());	
		NS_RELEASE(text);
		text =_rules_doc->CreateNodeWithText(p_rule_node, MozStr("PixHeight"), pimg->GetHeight());	
		NS_RELEASE(text);
		text =_rules_doc->CreateNodeWithText(p_rule_node, MozStr("HasAlphaMask"), pimg->GetHasAlphaMask());	
		NS_RELEASE(text);
		...
		text =_rules_doc->CreateNodeWithText(p_rule_node, MozStr("NaturalHeight"), height);	
		NS_RELEASE(text);
		text =_rules_doc->CreateNodeWithText(p_rule_node, MozStr("NaturalWidth"), width);	
		NS_RELEASE(text);

		nsColorMap* cmap = pimg->GetColorMap();
		if (cmap != nsnull)
		{
			text = _rules_doc->CreateNodeWithText(p_rule_node, MozStr("NumColors"), cmap->NumColors);	
			NS_RELEASE(text);
		}
		else
		{
			text = _rules_doc->CreateNodeWithText(p_rule_node, MozStr("NumColors"), -1);	
			NS_RELEASE(text);
		}
...
void CRuleCreator::CreateRuleNode_ChildTree(nsIDOMElement* p_dom_node, nsIDOMElement* p_rule_node)
{
	...
	if (CreateRuleNode_AddContent(p_dom_node, p_rule_node))
		return;
	...
	if (yes)
	{
		...
		for (PRInt32 idx=0; idx< cnt;idx++) 
		{
			...
			if (_rules_doc->IsWhiteSpace(child))
			{
				NS_RELEASE(child);
				continue;
			}
...
void CRuleCreator::CreateRuleNode_AddSiblings(nsIDOMElement* p_dom_node, nsIDOMElement* p_rule_node)
{
	...
	while (rv == NS_OK && prev != nsnull && _rules_doc->IsWhiteSpace(prev))
	{		
	...
	while(rv == NS_OK && next != nsnull && _rules_doc->IsWhiteSpace(next))
	{
	...
	if (rv == NS_OK && next != nsnull && !_rules_doc->IsWhiteSpace(next))
	{

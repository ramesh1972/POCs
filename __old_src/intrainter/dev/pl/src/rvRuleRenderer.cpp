...
II_RESULT CRuleRenderer::ProcessPickMeCommand(nsIDOMElement* p_matched_html_node, nsIDOMElement* p_processed_rule_node, PRBool p_content_has_changed)
{
  ...
  mAlertBrowser->SetApp(IAPP);
	mAlertBrowser->m_window_type = eWindowType_toplevel;
	mAlertBrowser->m_border_style = eBorderStyle_all;
	
	ON_THE_RUN(IIR_APP_EXITED)
  mAlertBrowser->Init(IPWW,IAPP->mAppShell,nsRect(0, 0, 400, 300));
	mAlertBrowser->mSubWinType = IIW_AlertView;

  // add the browser
	ON_THE_RUN(IIR_APP_EXITED)
  IPWW->AddBrowser(mAlertBrowser);

  // load the page
	mAlertBrowser->GoTo(url);

	II_RESULT wait_result = mAlertBrowser->WaitForDocumentToLoad();
	...
	ILOG << "Successfully Loaded Empty Document" << IINF;
	ILOG << "Continuing to Render!!!!" << IINF;
  ...
  if (!doc) 
	...
	mAlertBrowser->ArchivePage(m_rule_name);
	mAlertBrowser->mSubWinType = IIW_AlertView;

	mAlertBrowser->mDocIsLoaded = PR_TRUE; 
	NS_RELEASE(uri);
	NS_RELEASE(body);
	NS_RELEASE(aDocument);
	NS_RELEASE(mAlertBrowser);

...
function check_set_and_show_section_tree(rsection, deep)
{
	...
	make_container_plain(rsection);

	// remove bars for containers with one child
	destroy_bars_for_containers(rsection, deep);
...
// ================================================================================
function destroy_bars_for_containers(section, deep) {
    if (!is_root(section) && (section == null || section.real_type != "dock_container"))
        return false;
...
//        if (child.is_visible && !is_border(child))
//            num_children++;
...
//    if (num_children-1 == num_bars)
    ...
    if (deep)
        for (var idx = 0; idx < section.child_sections.length; idx++) {
            var child = section.child_sections[idx];
                destroy_bars_for_containers(child, deep);
        }
...
function remove_empty_containers(rsection) {
	...
	
	{
		...
		for (idx=0; idx< temp.child_sections.length; idx++)
		{
			...
			if (!is_border(child) && !is_bar(child))
			{
				num_sections++;
				sect_to_move = child;
			}
		...
		
		{
			...
			if (rsection.real_type == "container" && num_sections == 0)
			{
//					//alert(0.6);
					remove_tab(rsection);
			...
			else if ((rsection.real_type == "container" || rsection.real_type == "dock_container") && num_sections == 1) // then there is no need for the container
			{
					...
					if (rsection.tab_container != null)
					{
						...
						set_section_dimension(sect_to_move, sect_to_move.tab_container.left, sect_to_move.tab_container.top, sect_to_move.tab_container.width, sect_to_move.tab_container.height);
...
// ===========================================================================================
function make_container_plain(container) {
    if (container != null)
    {
        for (var idx = 0; idx < container.child_sections.length; idx++) 
        {
            var child = container.child_sections[idx];
            make_container_plain(child);

            // remove bars
            if (container.real_type == "dock_container" && is_bar(child)) {
    ...
    // remove borders
    if (container.real_type == "dock_container")
        destroy_section_borders(container);
}
...
function set_section_tree_children_dimensions(rsection, deep)
{
	...
	{
		// tabs, tab bars are laid out by a separate function rather than the standard functions
		if (is_tab_bar(rsection))
		{
			layout_tab_bar(rsection);
			layout_tab_bar_tabs(rsection);

...
nvp data_construct::get()
{
	...
	__processor::__instructions::pi_dispatch_link(nl);
...
{
	if (__instructions::ci_is_supported_construct_type(t->name()))
		return t;
...
//	if (ret_value != NULL)
	...
	ret_value = __processor::__instructions::pi_register_data_construct("ret_value", ret, "dvp", this);
}
